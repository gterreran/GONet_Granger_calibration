...
"""
Centralized Dash component IDs used by the GUI layout and callbacks.

Keeping component IDs in one module avoids string drift between
:mod:`~grid_calibration.gui.layout`, the general callback modules, and the
interactive step-specific callback modules.  Most IDs are plain strings.  Step
buttons and step option widgets use Dash pattern-matching IDs generated by
:func:`step_button_id` and :func:`step_dropdown_id`.
"""
...
"""ID of the store containing the latest completed or active workflow step."""
STORE_CONTROL_STEPS = "control-row-steps-store"
"""ID of the store containing the ordered control-row step keys."""
STORE_RUN_STEP = "run-step-store"
"""ID of the store used to request execution of a batch pipeline step."""
STORE_STEP_REQUEST = "step-request-store"
"""ID of the store containing the currently requested step action."""
STORE_STEP_RESULT = "step-result-store"
"""ID of the store containing the most recent step execution result."""

# Controls
BTN_EXIT = "btn-exit"
"""ID of the button used to exit the desktop GUI."""

# Step run buttons (keep existing names to avoid breaking callbacks)
def step_button_id(step: str) -> dict[str, str]:
    """
    Return the pattern-matching ID for a step run button.

    Parameters
    ----------
    step : :class:`str`
        Workflow step key from
        :data:`~grid_calibration.gui.workflow.registry.ORDERED_STEPS`.

    Returns
    -------
    dict[str, str]
        Dash pattern-matching ID with ``type="button"`` and the supplied step
        key.
    """
    return {"type": "button", "step": step}

# Step right-side widgets
def step_dropdown_id(step: str) -> dict[str, str]:
    """
    Return the pattern-matching ID for a step option/dropdown widget.
    ...
    -------
    dict[str, str]
        Dash pattern-matching ID with ``type="options"`` and the supplied step
        key.
    """
...
MODELING_TANGENTIAL_HARMONIC_RADIAL_DEGREE_ID = "modeling-tangential-harmonic-radial-degree"
MODELING_TANGENTIAL_HARMONIC_ORDER_ID = "modeling-tangential-harmonic-order"
MODELING_TWIST_KIND_ID = "modeling-twist-kind"
MODELING_TWIST_SCALE_ID = "modeling-twist-scale"
