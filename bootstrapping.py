...
def fit_parametric_spoke_spline(
    x: np.ndarray,
    y: np.ndarray,
    smoothing: float = DEFAULT_BOOSTRAPPING_PARAMS["spoke_spline_smoothing"],
    spline_order: int = 3,
):
...
def sample_spline(tck, n_samples: int = DEFAULT_BOOSTRAPPING_PARAMS["spoke_sample_count"]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def estimate_inner_cutoff_pix(
    ...
    cutoff_nom = max(rmin_nom - DEFAULT_BOOSTRAPPING_PARAMS["inner_cutoff_margin_deg"], 0.0)
    ...
    pp = pix_r[valid]
    deg = min(DEFAULT_BOOSTRAPPING_PARAMS["inner_cutoff_poly_degree"], max(1, rr.size - 1))
...
def _reject_ambiguous_best(cand: np.ndarray, score: np.ndarray, rem_xy: np.ndarray) -> bool:
    ...
    return (
        ((score_diff < DEFAULT_BOOSTRAPPING_PARAMS["ambiguity_score_tol"]) or (score_ratio < DEFAULT_BOOSTRAPPING_PARAMS["ambiguity_ratio_tol"]))
        and (point_sep < DEFAULT_BOOSTRAPPING_PARAMS["ambiguity_point_sep_px"])
    )
...
def choose_inward_candidate(
    ...
    cand = np.where(
        ...
        & (dist_px <= DEFAULT_BOOSTRAPPING_PARAMS["spoke_extrap_tol_px"])
        & (angle_deg <= DEFAULT_BOOSTRAPPING_PARAMS["inward_aperture_deg"])
    )[0]

    if cand.size == 0:
        return None

    score = candidate_score(dist_px[cand], angle_deg[cand], DEFAULT_BOOSTRAPPING_PARAMS["spoke_extrap_tol_px"], DEFAULT_BOOSTRAPPING_PARAMS["inward_aperture_deg"])
...
def choose_outward_candidate(
    ...
    cand = np.where(
        side_mask
        & outward_mask
        & (d_parallel > DEFAULT_BOOSTRAPPING_PARAMS["outward_forward_min_px"])
        & (d_perp <= DEFAULT_BOOSTRAPPING_PARAMS["outward_perp_tol_px"])
        & (angle_deg <= DEFAULT_BOOSTRAPPING_PARAMS["outward_aperture_deg"])
    )[0]

    if cand.size == 0:
        return None

    score = candidate_score(d_perp[cand], angle_deg[cand], DEFAULT_BOOSTRAPPING_PARAMS["outward_perp_tol_px"], DEFAULT_BOOSTRAPPING_PARAMS["outward_aperture_deg"])
    order = np.argsort(score)
    cand = cand[order]
    score = score[order]
...
def bootstrap_spoke_pair(
    ...
    *,
    spoke_tol_px: float = DEFAULT_BOOSTRAPPING_PARAMS["spoke_final_tol_px"],
    ...
    initial_keep = (dist0 <= DEFAULT_BOOSTRAPPING_PARAMS["spoke_initial_pull_tol_px"]) & (pts_radius >= cutoff_pix)
    ...
    for _ in range(DEFAULT_BOOSTRAPPING_PARAMS["max_growth_steps"]):
...
def fit_circle_radial_model(
    ...
    r_pix: np.ndarray,
    n_harmonics: int = DEFAULT_BOOSTRAPPING_PARAMS["circle_fourier_harmonics"],
) -> np.ndarray:
...
def revert_circle_outliers(
    ...
    center_xy: np.ndarray,
    r_deg: float,
    mad_threshold: float = DEFAULT_BOOSTRAPPING_PARAMS["circle_outlier_mad_threshold"],
) -> int:
    """
    Remove inconsistent assignments on one circle using a Fourier circle model.
    """
    n_harmonics = DEFAULT_BOOSTRAPPING_PARAMS["circle_fourier_harmonics"]   
    total = 0
...
def assign_intersection_radii_from_spokes(
    ...
    while True:
        ...
        next_upper_rings = [r for r in np.arange(inner_ring, DEFAULT_BOOSTRAPPING_PARAMS["max_nominal_r_deg"] + DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"], DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"])
                            if not np.any(np.isclose(unique_rings, r))]

        next_lower = None if np.isclose(inner_ring, DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"]) else inner_ring - DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"]
        ...
        for theta in unique_spokes:
            ...
            snapped = DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"] * np.round(r_est / DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"])
...
def assign_circle_only_points(
    ...
    for r_deg in unique_circles:
        ...
        if np.sum(valid) < 2 * DEFAULT_BOOSTRAPPING_PARAMS["circle_fourier_harmonics"] + 2:
            continue
        ...
        good = np.abs(r_all[unassigned_mask] - r_pred) <= DEFAULT_BOOSTRAPPING_PARAMS["circle_only_mad_factor"] * mad
        ...
        data.append(
            {
                ...
                "r": r,
                "theta": theta,
                "circle_index": int(np.round(nominal_points.r_nom_deg[pos] / DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"])),
                "spoke_index": int(np.round(nominal_points.theta_nom_deg[pos] / DEFAULT_BOOSTRAPPING_PARAMS["grid_step_deg"])),
